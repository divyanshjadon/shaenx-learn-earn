...
export function BountyCard({
  id, title, description, reward, rewardType, deadline,
  applicants, category, skills, company, featured = false,
}: BountyCardProps) {
  return (
    <div
      className={`neon-card-hover p-6 flex flex-col h-full ${
    ...
    >
      ...
      <div className="flex items-start justify-between gap-4 mb-4">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <Badge variant="outline" className={category === "Web3" ? "badge-web3" : "badge-web2"}>
              {category}
            </Badge>
          </div>
        </div>
        {featured && (
          <Badge className="bg-gradient-accent text-accent-foreground border-0">Featured</Badge>
        )}
      </div>
      ...
      <div className="flex flex-wrap gap-2 mb-4">
        {skills.slice(0, 3).map((skill) => (
          <Badge key={skill} variant="secondary" className="text-xs font-mono font-normal">{skill}</Badge>
        ))}
        {skills.length > 3 && (
          <Badge variant="secondary" className="text-xs font-normal">+{skills.length - 3}</Badge>
        )}
      </div>
      ...
      <div className="flex items-center justify-between pt-4 border-t border-border/50">
        <div className="flex items-center gap-2">
          ...
          <span className="font-display font-bold text-lg">
            {reward} <span className="text-sm font-normal text-muted-foreground font-mono">{rewardType}</span>
          </span>
        </div>
        <Link to={`/explore/${id}`}>
